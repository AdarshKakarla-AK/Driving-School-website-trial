  ...
  return (
    <div className="mx-auto max-w-7xl px-4 pb-20 pt-28 sm:px-6">
      <div className="max-w-2xl">
        <span className="text-xs font-bold uppercase tracking-[0.2em] text-brand-600">Courses & Pricing</span>
        <h1 className="font-display mt-3 text-4xl font-bold tracking-tight text-ink-900 sm:text-5xl">Pick your perfect course</h1>
        <p className="mt-4 text-ink-500">
          Every package includes a certified instructor, dual-control car, attendance tracking and a certificate on completion. EMI available on most courses.
        </p>
      </div>
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
              ...
              <div className="flex flex-col rounded-2xl border border-ink-100 bg-paper p-6">
                <div className="flex items-baseline gap-2">
                  <span className="font-display text-4xl font-bold text-ink-900">{formatINR(p.price)}</span>
                ...
                <div className="mt-auto space-y-2 pt-5">
                  <Link href={`/book?pkg=${p.slug}`} className={buttonClasses("primary", "lg", "w-full")}>
...
function InfoChip({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-xl bg-ink-50 p-3">
      <div className="flex items-center gap-1.5 text-brand-600">{icon}</div>
